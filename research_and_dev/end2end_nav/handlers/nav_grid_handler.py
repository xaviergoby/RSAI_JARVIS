...
class NavigationGridHandler:
	...
	def __init__(self, mesh_grid_opt):
		
		self.mesh_grid_opt = mesh_grid_opt
		self.px_x_axes_pnts = np.linspace(*self.mesh_grid_opt["x_grid_pnts"])
		self.px_y_axes_pnts = np.linspace(*self.mesh_grid_opt["y_grid_pnts"])
		self.num_of_px_pnts_on_axes = len(self.px_x_axes_pnts)
		self.mesh_grid_px_x_pnts, self.mesh_grid_px_y_pnts = np.meshgrid(self.px_x_axes_pnts, self.px_y_axes_pnts)
		...
		self.mesh_grid_x_pnts, self.mesh_grid_y_pnts = np.meshgrid(self.ego_cent_rf_grid_x_axes_pnts,
	...
	def get_lmc_nn_grid_pnt_idxs(self, lmc_px_x, lmc_px_y):  # 1.1
		...
		lmc_nn_grid_pnt_coords_idxs = np.argwhere((self.mesh_grid_px_x_pnts == lmc_nn_px_x) &
		                                          (self.mesh_grid_px_y_pnts == lmc_nn_px_y))[0]
...
if __name__ == "__main__":
	...
	mgrid = NavigationGridHandler(settings.MESH_GRID_OPTIONS[1])
	...
	r = draw_sq_grid_lines_on_img_v1(1, mgrid.px_x_axes_pnts.astype(int).tolist(), mgrid.px_y_axes_pnts.astype(int).tolist())
	print(r[0])
	print(r[1])
